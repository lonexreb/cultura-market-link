...
import { Activity, Zap, DollarSign, Clock, Cpu, Database, Key, Workflow, FileText, Rocket, Network, Play, Square, X, Settings, Box } from 'lucide-react';
...
const Index = () => {
  ...
  // Listen for brick-to-node events
  useEffect(() => {
    const handleCreateNodeFromBrick = (event: CustomEvent) => {
      const { nodeType, position, id, data } = event.detail;
      ...
      const newNode = {
        ...
        data: {
          ...
          outputData: undefined,
          onShowOutputData: () => handleShowNodeOutput(id),
        },
      };

      setNodes((nds) => {
        // Check if node already exists to prevent duplicates
        const existingNode = nds.find(n => n.id === id);
        if (existingNode) {
          console.log('Node already exists, skipping:', id);
          return nds;
        }
        
        console.log('✅ Creating node from brick:', newNode);
  ...
  useEffect(() => {
    if (nodes.length > 0) {
      console.log('🔄 Syncing existing workflow nodes to bricks:', nodes.length);
      
      // Dispatch event to create bricks from existing nodes
      const syncEvent = new CustomEvent('syncNodesToBricks', {
        detail: { nodes }
      });
  ...
  const getNodeLabel = (type: string) => {
    switch (type) {
      ...
      case 'github_mcp': return 'GitHub MCP';
      default: return 'Node';
    }
  };
  ...
  const getNodeDescription = (type: string) => {
    switch (type) {
      ...
      case 'github_mcp': return 'GitHub operations with MCP protocol integration';
    ...
    <ApiKeysProvider>
      <SchemasProvider>
        ...
        <div className="flex-1 relative">
          ...
          {/* Tab Content Panel */}
          <AnimatePresence>
            {activeTab !== 'workflow' && activeTab !== 'knowledge-graph' && activeTab !== 'networking' && activeTab !== 'brick-builder' && (
              <motion.div
      ...
      <AnimatePresence>
        {!isNodePanelOpen && activeTab !== 'knowledge-graph' && activeTab !== 'networking' && activeTab !== 'brick-builder' && (
          <motion.button
      ...
      {/* Node Panel */}
      {activeTab !== 'knowledge-graph' && activeTab !== 'networking' && activeTab !== 'brick-builder' && (
        <NodePanel 
